...
class KCFTracker:
	def __init__(self, hog=False, fixed_window=True, multiscale=False):
		self.direction = {'x': 'zero', 'y': 'zero'}
		self.direction_temp = {'x': 'zero', 'y': 'zero'}
	...
	def getFeatures(self, image, inithann, scale_adjust=1.0):
		...
		# while (self._tmpl_sz[0] // 4) % 2 != 0:
		# 	self._tmpl_sz[0] += 1

		# while (self._tmpl_sz[1] // 4) % 2 != 0:
		# 	self._tmpl_sz[1] += 1

		if(z.shape[1]!=self._tmpl_sz[0] or z.shape[0]!=self._tmpl_sz[1]):
			z = cv2.resize(z, tuple(self._tmpl_sz))

		# print('z.shape: {}, tmpl_sz: {}'.format(z.shape, self._tmpl_sz))

		if(self._hogfeatures):
			mapp = {'sizeX':0, 'sizeY':0, 'numFeatures':0, 'map':0}
			mapp = fhog.getFeatureMaps(z, self.cell_size, mapp)
			# print("mapp[sizeY]: {}, mapp[sizeX]: {}".format(mapp['sizeY'], mapp['sizeX']))
	...
	# def init(self, roi, image):
		...
		# print("tmpl: {}, prob: {}, alphaf: {}".format(self._tmpl.shape, self._prob.shape, self._alphaf.shape))
	...
	def update(self, image):
		...
		# print("({:+.4f}, {:+.4f}) =>({:+.4f}, {:+.4f}) peak {:.2f}".format(loc[0], loc[1], self.location[0], self.location[1], peak_value))

		# dir_x = 'left' if self.location[0] < 0 else 'right'
		# dir_y = 'up' if self.location[1] < 0 else 'down'

		# if self.direction_temp['x'] != dir_x:
		# 	self.dir_x_count = 0
		# 	self.direction_temp['x'] = dir_x
		# else:
		# 	self.dir_x_count += 1

		# if self.dir_x_count > 10:
		# 	self.direction['x'] = self.direction_temp['x']

		# if self.direction_temp['y'] != dir_y:
		# 	self.dir_y_count = 0
		# 	self.direction_temp['y'] = dir_y
		# else:
		# 	self.dir_y_count += 1

		# if self.dir_y_count > 10:
		# 	self.direction['y'] = self.direction_temp['y']

		# print(self.direction['x'], 'and', self.direction['y'])

		# if (self.direction['x'] != dir_x) or (self.direction['y'] != dir_y):
		...
		if (self.scale_step != 1):
			...
			# 	new_peak_value1 = 0.96 * new_peak_value1
			# 	new_peak_value2 = 0.96 * new_peak_value2
			# else:
			# 	new_peak_value1 = 0.96 * new_peak_value1
			# 	new_peak_value2 = 0.96 * new_peak_value2
			# print("{:.2f} vs {:.2f} vs {:.2f}".format(peak_value, new_peak_value1, new_peak_value2))

			new_peak_value1 = 0.96 * new_peak_value1
			new_peak_value2 = 0.96 * new_peak_value2
			new_peak_value3 = 0.91 * new_peak_value3
			new_peak_value4 = 0.91 * new_peak_value4
		...
		# print("loc: ({:+.4f},{:+.4f}), peak_value: {}:{:.2f}, scale: {:.2f}".format(loc[0], loc[1], peak_index, peak_value, self._scale))
