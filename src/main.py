        ...
        if tracking_window['start'] == True:
            if((tracking_window['x2'] - tracking_window['x1']) > MIN_SELECTION_WIDTH) and ((tracking_window['y2'] - tracking_window['y1']) > MIN_SELECTION_HEIGHT):
                ...
                if kcf_tracker:
                    ...
                    if zoom:
                        kcf_tracker.no_zoom_width = int(initial_width / zoom.current_zoom)
                        kcf_tracker.no_zoom_height = int(initial_height / zoom.current_zoom)
                    else:
                        kcf_tracker.no_zoom_width = initial_width
                        kcf_tracker.no_zoom_height = initial_height
        ...
        if tracking_processing_flag is True:
            ...
            if kcf_tracker:
                ...
                if args['autozoom'] and zoom.is_zooming is not True:
                    next_zoom = zoom.find_next_auto_zoom(kcf_tracker.mean_width)
